...
function wc_mpesa_gateway_init()
{
    ...
    class WC_MPESA_Gateway extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->method_description = ($env == 'live') ? __('Receive payments via Safaricom M-PESA', 'woocommerce') : __('<h3>Pre-requisites</h3>
            <article>
                <ol>
			...
			<p>You should probably run a few tests first, then <a href="' . admin_url('edit.php?post_type=mpesaipn&page=wc_mpesa_live') . '">click here</a> for instructions on how to go live.</p>
			<p>Remember to <a title="' . __('Navigate to page and click Save Changes', 'woocommerce') . '" href="' . admin_url('options-permalink.php') . '">flush your rewrite rules</a>.</p>');
...
You will receive a confirmation message shortly thereafter.', 'woocommerce'),
                ...
                'instructions'       => array(
                    ...
                    'default'     => __('Thank you for buying from us. You will receive a confirmation message from M-PESA shortly.', 'woocommerce'),
                    'desc_tip'    => true,
                ),
                // 'account' => array(
        ...
        public function process_payment($order_id)
        {
            ...
            if ($result) {
                ...
                if (isset($result['errorCode'])) {
                    $error_message = 'M-PESA Error ' . $result['errorCode'] . ': ' . $result['errorMessage'];
