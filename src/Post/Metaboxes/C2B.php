...
class C2B
{
    ...
    public static function mpesa_mb_sm()
    {
        ...
        //add_meta_box('c2b-payment-payment_status', 'Incase M-PESA timed out', [new self, 'mpesa_status'], [ 'mpesaipn', 'shop_order' ], 'side', 'low');
        add_meta_box('c2b-payment-payment_create', 'Paid For Via M-PESA?', [new self, 'mpesa_payment'], 'shop_order', 'side', 'low');
    }
    ...
    public static function mpesa_payment($post)
    {
        echo '<table class="form-table" >
            <tr valign="top" >
                <td>
                    You can manually register a payment via M-PESA after saving this order.
                </td>
            </tr>
            <tr valign="top" >
                <td>
                    <a href="' . admin_url('post-new.php?post_type=mpesaipn&order=' . $post->ID . '') . '" class="button">Add New M-PESA Payment</a>
                </td>
            </tr>
        </table>';
    }
    ...
    public static function mpesa_status($post)
    {
        ...
        echo '<table class="form-table" >
            ...
            <tr valign="top" >
                <td>
                    <button type="submit" id="mpesaipn_status_checker" name="mpesaipn_status" class="button button-large">Check Payment Status</button>
                </td>
            </tr>
        </table>';
    }
    ...
    public static function order_details($post)
    {
        ...
        $new = wc_get_order($order)
        ? ''
        : ' <a href="' . admin_url('post-new.php?post_type=shop_order') . '" class="button">Add New Manual Order</a>';
    ...
    public static function payment_details($post)
    {
        ...
        <p>Enter M-PESA confirmation code received and set status for order with <b>Request ID: <?php echo $request; ?></b>.</p>
        <?php echo '<p>M-PESA Receipt Number <input type="text" name="receipt" value="' . esc_attr($receipt) . ' " /></p>'; ?>
